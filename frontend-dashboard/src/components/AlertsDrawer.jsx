      ...
      <div style={{
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'18px 20px 14px', borderBottom:'1px solid var(--border-2)' }}>
          <div style={{ display:'flex', alignItems:'center' }}>
            <span style={{ fontFamily:'Rajdhani,sans-serif', fontSize:16, fontWeight:700, letterSpacing:'.1em', color:'var(--text)' }}>ALERTAS ACTIVAS</span>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'14px 16px' }}>
          {criticas.length > 0 && (
            <>
              <p style={{ fontSize:10, letterSpacing:'.1em', color:'var(--text-muted)', fontWeight:600, textTransform:'uppercase', marginBottom:8 }}>Críticas</p>
          ...
          {advertencias.length > 0 && (
            <>
              <p style={{ fontSize:10, letterSpacing:'.1em', color:'var(--text-muted)', fontWeight:600, textTransform:'uppercase', marginBottom:8 }}>Advertencias</p>
...
function AlertNotif({ alerta, onGo }) {
  ...
  return (
    <div style={{ background:'var(--bg-card)', border:'1px solid var(--border-2)', borderLeft:`3px solid ${bl}`, borderRadius:6, padding:'12px 14px', marginBottom:10 }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:4 }}>
        ...
        <span style={{ fontSize:10, color:'var(--text-muted)' }}>{alerta.tiempo}</span>
      </div>
      <div style={{ fontSize:12, color:'var(--text-2)', lineHeight:1.5, marginBottom:10 }}>{alerta.msg}</div>
      <button onClick={onGo} style={{
        fontSize:11, fontWeight:600, letterSpacing:'.05em', padding:'5px 12px',
        borderRadius:3, border:'1px solid var(--border)', background:'var(--bg-card-2)', color:'#1a7a50',
