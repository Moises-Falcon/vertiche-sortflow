...
export default function StageModal({ stageKey, onClose }) {
  ...
  return (
    ...
    >
      <div style={{
        background:'var(--bg-card)', color:'var(--text)', borderRadius:10,
        width:720, maxWidth:'95vw', maxHeight:'90vh',
        overflowY:'auto', boxShadow:'0 20px 60px rgba(0,0,0,0.55)',
        fontFamily:'Inter,sans-serif',
      }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'18px 22px 14px', borderBottom:'1px solid var(--border-2)', position:'sticky', top:0, background:'var(--bg-card)', zIndex:1 }}>
          <div style={{ display:'flex', alignItems:'center' }}>
            <span style={{ fontFamily:'Rajdhani,sans-serif', fontSize:18, fontWeight:700, letterSpacing:'.1em', color:'var(--text)' }}>{modal.title}</span>
        ...
        <div style={{ padding:'20px 22px', background:'var(--bg-card)', color:'var(--text)' }}>
          <div style={{ fontSize:10, letterSpacing:'.1em', color:'var(--text-muted)', fontWeight:500, textTransform:'uppercase', marginBottom:4 }}>{modal.kpiLabel}</div>
          ...
          {modal.stats2 && (
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', border:'1px solid var(--border-2)', borderRadius:4, overflow:'hidden', marginBottom:12 }}>
              {modal.stats2.map((s,i) => (
                <div key={i} style={{ padding:'12px 14px', borderRight: i===0 ? '1px solid var(--border-2)' : 'none', background:'var(--bg-card)' }}>
                  <div style={{ fontSize:10, letterSpacing:'.08em', color:'var(--text-muted)', fontWeight:500, textTransform:'uppercase', marginBottom:4 }}>{s.label}</div>
          ...
          {modal.stats4 && (
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:12 }}>
              {modal.stats4.map((s,i) => (
                <div key={i} style={{ background:'var(--bg-card-2)', border:'1px solid var(--border)', borderRadius:3, padding:'11px 13px' }}>
                  <div style={{ fontSize:10, letterSpacing:'.08em', color:'var(--text-muted)', marginBottom:3, textTransform:'uppercase', fontWeight:500 }}>{s.label}</div>
          ...
          {modal.donut && (
            <>
              <div style={{ fontSize:10, letterSpacing:'.1em', color:'var(--text-muted)', fontWeight:500, textTransform:'uppercase', marginBottom:10 }}>DISTRIBUCIÓN DE CARGA</div>
          ...
          {modal.bars && modal.bars.map((bar, i) => (
            <div key={i} style={{ marginBottom:10 }}>
              {i===0 && <div style={{ fontSize:10, letterSpacing:'.1em', color:'var(--text-muted)', fontWeight:500, textTransform:'uppercase', marginBottom:10 }}>
              ...
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:5, fontSize:12 }}>
                <span style={{ color:'var(--text-2)' }}>{bar.name}</span>
                <span style={{ color: bar.valColor || '#555', fontWeight:500 }}>{bar.pct}</span>
              </div>
              <div style={{ height:5, background:'var(--bg-sunken)', borderRadius:2, overflow:'visible', position:'relative', marginBottom: bar.thresholds ? 18 : 0 }}>
                ...
                {bar.thresholds?.map((t,ti) => (
                  <div key={ti} style={{ position:'absolute', top:-3, left:`${t.pos}%`, width:1, height:11, background:'rgba(0,0,0,0.18)' }}>
                    <span style={{ position:'absolute', top:10, fontSize:9, color:'var(--text-muted)', transform:'translateX(-50%)', whiteSpace:'nowrap' }}>{t.lbl}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {modal.stacked && (
            <>
              <div style={{ fontSize:10, letterSpacing:'.1em', color:'var(--text-muted)', fontWeight:500, textTransform:'uppercase', marginBottom:10 }}>
              ...
              <div style={{ display:'flex', gap:14, fontSize:11, color:'var(--text-muted)', marginTop:5, flexWrap:'wrap' }}>
          ...
          {modal.tablaBahias && (
            <>
              <div style={{ height:1, background:'var(--border-2)', margin:'16px 0' }} />
              <div style={{ fontSize:10, letterSpacing:'.1em', color:'var(--text-muted)', fontWeight:500, textTransform:'uppercase', marginBottom:10 }}>DETALLE POR BAHÍA</div>
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12, background:'var(--bg-card)' }}>
                <thead>
                  <tr>
                    {['BAHÍA','PREPACKS','PP/MIN','ESTADO'].map(h => (
                      <th key={h} style={{ fontSize:10, letterSpacing:'.08em', color:'var(--text-muted)', fontWeight:500, textTransform:'uppercase', textAlign:'left', padding:'0 10px 7px 0', borderBottom:'1px solid var(--border-2)' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {modal.tablaBahias.map((row,i) => (
                    <tr key={i}>
                      <td style={{ padding:'8px 10px 8px 0', borderBottom:'1px solid var(--border-2)', color:'var(--text)', fontWeight:500 }}>{row.bahia}</td>
                      <td style={{ padding:'8px 10px 8px 0', borderBottom:'1px solid var(--border-2)', color:'var(--text-2)' }}>{row.prepacks}</td>
                      <td style={{ padding:'8px 10px 8px 0', borderBottom:'1px solid var(--border-2)', color:'var(--text-2)' }}>{row.ppm}</td>
                      <td style={{ padding:'8px 10px 8px 0', borderBottom:'1px solid var(--border-2)' }}>
          ...
          {modal.tablaDiscrepancias && (
            <>
              <div style={{ height:1, background:'var(--border-2)', margin:'16px 0' }} />
              <div style={{ fontSize:10, letterSpacing:'.1em', color:'var(--text-muted)', fontWeight:500, textTransform:'uppercase', marginBottom:10 }}>TIENDAS CON DISCREPANCIA</div>
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
                ...
                <tbody>
                  {modal.tablaDiscrepancias.map((row,i) => (
                    <tr key={i}>
                      <td style={{ padding:'8px 10px 8px 0', color:'var(--text)', fontWeight:500 }}>#{row.tienda}</td>
                      <td style={{ padding:'8px 10px 8px 0', color:'var(--text-2)' }}>{row.esperadas}</td>
                      <td style={{ padding:'8px 10px 8px 0', color:'var(--text-2)' }}>{row.enviadas}</td>
